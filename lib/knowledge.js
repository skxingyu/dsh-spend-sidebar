...
/**
 * Model pricing catalog ported from kenz1117/dsh-ui-usage-billing
 * (MODEL_CATALOG via tools/build-catalog.mjs — regenerate against a fresh
 * checkout of that repo to pick up vendor repricings). Rows keep their
 * native currency; `aliases` maps logged model ids to catalog keys.
 */
import modelCatalogData from "./model-catalog-data.json" with { type: "json" };
export const MODEL_CATALOG = modelCatalogData;
export const USD_TO_CNY_CATALOG = modelCatalogData.usdToCny;
...
/**
 * Auto-generated pricing rows for one provider. Two sources, catalog first:
 *
 *   1. `model-catalog-data.json` — ported from kenz1117/dsh-ui-usage-billing's
 *      MODEL_CATALOG (regenerate with tools/build-catalog.mjs). Rows keep
 *      their NATIVE currency (`currency: "CNY" | "USD"`) and carry an
 *      optional peak/off-peak `schedule`. This table is the fresher of the
 *      two (it tracks vendor repricings, e.g. DeepSeek 2026-09-10).
 *   2. `PROVIDER_KNOWLEDGE[provider].rates` — the hand-maintained base
 *      (USD-assumed). Used only for rows the catalog does not cover.
 *
 * Matching by raw logged model id goes through MODEL_ALIASES first: logs
 * carry full ids (`deepseek-v4-flash`) while the catalog keys are short
 * (`flash`). Returns [] when the provider has no rate table at all.
 */
export function autoRatesFor(provider) {
  const canonical = normalizeProvider(provider);
  const results = [];
  const covered = new Set();
  const catalogProviders = MODEL_CATALOG.providers[canonical];
  if (catalogProviders !== undefined) {
    for (const rate of catalogProviders) {
      results.push({ ...rate, provider, auto: true });
      covered.add(rate.model);
    }
    // Alias expansion: the catalog keys are billing keys (`flash`), while
    // session logs carry vendor ids (`deepseek-v4-flash`). Emit one row per
    // logged id that maps onto a catalog key, so resolvePrice's exact match
    // finds the row without touching the aggregation path. Ids already equal
    // to a key are covered by the catalog rows themselves.
    for (const [loggedId, catalogKey] of Object.entries(MODEL_CATALOG.aliases)) {
      if (covered.has(loggedId) || loggedId === catalogKey) continue;
      const row = catalogProviders.find((candidate) => candidate.model === catalogKey);
      if (row === undefined) continue;
      results.push({ ...row, model: loggedId, provider, auto: true });
      covered.add(loggedId);
    }
  }
  const entry = PROVIDER_KNOWLEDGE[canonical];
  if (entry?.rates !== undefined) {
    for (const rate of entry.rates) {
      // A catalog row for the same billing key wins (it is the fresher
      // source); knowledge rows that the catalog does not model — e.g.
      // date-gated legacy pricing — still apply.
      const catalogKey = MODEL_CATALOG.aliases[rate.model] ?? rate.model;
      if (covered.has(rate.model) || covered.has(catalogKey)) continue;
      results.push({ ...rate, provider, auto: true });
    }
  }
  return results;
