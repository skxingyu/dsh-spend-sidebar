...
/**
 * Estimated cost for one call, expressed in the display currency (rates are
 * per million tokens).
 *
 * Each pricing row may declare its NATIVE `currency` ("USD" | "CNY"); rows
 * without one are assumed to already be in the display currency (the
 * hand-maintained knowledge rows predate per-row currency and are written in
 * it). Conversion goes through `options.rates` — `{ USD, CNY }`, CNY per 1
 * USD — so a CNY-priced DeepSeek row displayed in USD divides by the rate
 * while a USD row displayed in CNY multiplies by it. Native-currency rows
 * that already match the display currency pass through untouched.
 */
export function costOf(sample, pricing, defaultPricing, conversion) {
  const price = resolvePrice(sample.model, sample.provider, pricing, defaultPricing, sample.time);
  let { inputPerMillion, cacheReadPerMillion, cacheWritePerMillion, outputPerMillion } = price;
  if (conversion !== undefined) {
    const factor = conversion.factorFor(price.currency);
    inputPerMillion *= factor;
    cacheReadPerMillion *= factor;
    cacheWritePerMillion *= factor;
    outputPerMillion *= factor;
  }
  const perMillion = 1e6;
  return {
    cost: (sample.inputTokens * inputPerMillion
      + sample.cacheReadTokens * cacheReadPerMillion
      + sample.cacheWriteTokens * cacheWritePerMillion
      + sample.outputTokens * outputPerMillion) / perMillion,
    costInput: sample.inputTokens * inputPerMillion / perMillion,
    costCacheRead: sample.cacheReadTokens * cacheReadPerMillion / perMillion,
    costCacheWrite: sample.cacheWriteTokens * cacheWritePerMillion / perMillion,
    costOutput: sample.outputTokens * outputPerMillion / perMillion,
  };
}

/**
 * Currency converter for mixed-currency price tables: maps a row's native
 * currency to the display currency. `rates` is `{ USD, CNY }` (each entry =
 * that currency's value per 1 USD, e.g. CNY ≈ 6.79); `display` is the target
 * currency code. Rows priced in the display currency convert by 1.
 *
 * native → display: `display = native / rates[native] × rates[display]`
 * (both entries are "per 1 USD", so dividing out the native quote then
 * applying the display quote is native × rates[display] / rates[native]).
 * Example: 2 CNY → USD is 2 / 6.79 × 1.
 */
export function currencyConverter(rates, display) {
  if (rates === undefined || display === undefined) return undefined;
  const displayPerUsd = rates[display];
  if (typeof displayPerUsd !== "number" || !(displayPerUsd > 0)) return undefined;
  return {
    /** Factor multiplying a native-currency price into the display currency. */
    factorFor(native) {
      if (native === undefined || native === null || native === display) return 1;
      const nativePerUsd = rates[native];
      if (typeof nativePerUsd !== "number" || !(nativePerUsd > 0)) return 1;
      return displayPerUsd / nativePerUsd;
    },
  };
}
...
export function buildStats(calls, pricing, defaultPricing, options = {}) {
  ...
  /** Native-currency -> display-currency conversion; undefined when the host
   * supplies no rates (every row is then taken at face value). */
  const conversion = currencyConverter(options.rates, options.currency);
  ...
  for (const sample of calls) {
    const cost = costOf(sample, pricing, defaultPricing, conversion);
