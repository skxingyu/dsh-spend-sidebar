...
/* The sidebar footer slot (sidebar.footer.action) is a plain flex ROW, so a
 * second plugin registering the same slot lands beside this card and both get
 * squeezed: this one truncates its figures. Stack them instead.
 * :has() reaches the shared parent from our own node, keeping the fix local to
 * this plugin — the other plugin needs no change, and the rule never matches
 * when this card is not in the slot. The child rule gives each card a full row;
 * without it they would still share the wrapped line.
 * [class$="_footerActions"] matches the CSS-module class without depending on
 * its build hash. */
[class$="_footerActions"]:has(.dsu-widget){flex-wrap:wrap}
[class$="_footerActions"]:has(.dsu-widget) > *{flex:1 1 100%;min-width:0}
`;
